...
int32_t loopback_tcps(uint8_t sn, uint16_t port, uint8_t* buf, uint8_t ip_ver)
{
   ...
   switch(getSn_SR(sn))
   {
      ...
      case SOCK_CLOSED:
         ...
         {
             tmp = socket(sn,Sn_MR_DUALT,port,0x00);
         }
         else
         {
...
//          ret = connect(sn, destip, destport, sizeof(destip)/sizeof(uint8_t)); /* Try to connect to TCP server(Socket, DestIP, DestPort) */
//       else
//          ret = connect(sn, destip, destport, sizeof(destip)/sizeof(uint8_t));
...
//    	  close(sn);
         ...
         {
             tmp = socket(sn,Sn_MR_DUALT,any_port++,0x00);
         }
         else
         {
...
int32_t loopback_udps(uint8_t sn, uint8_t* buf, uint16_t port, uint8_t ip_ver)
{
   ...
   switch(getSn_SR(sn))
   {
      ...
      case SOCK_CLOSED:
         ...
         {
             tmp = socket(sn,Sn_MR_DUALU,port,0x00);
         }
         else
         {
