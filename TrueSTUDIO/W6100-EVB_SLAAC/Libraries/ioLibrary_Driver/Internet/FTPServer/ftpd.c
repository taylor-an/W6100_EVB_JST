    	...
    	case SOCK_ESTABLISHED :
   		...
   		case SOCK_INIT :
   			...
   			if(ftp.dsock_mode == PASSIVE_MODE){
				...
				ret = connect(DATA_SOCK, remote_ip.cVal, remote_port, sizeof(remote_ip.cVal)/sizeof(uint8_t));
   				if(ret != SOCK_OK){
#if defined(_FTP_DEBUG_)
...
char proc_ftpd(char * buf)
{
	...
	switch (cmdp - commands)
	{
		...
		case STOR_CMD:
			...
			if((ret = connect(DATA_SOCK, remote_ip.cVal, remote_port, &addr_len)) != SOCK_OK){
#if defined(_FTP_DEBUG_)
				printf("%d:Connect error\r\n", DATA_SOCK);
#endif
				return ret;
