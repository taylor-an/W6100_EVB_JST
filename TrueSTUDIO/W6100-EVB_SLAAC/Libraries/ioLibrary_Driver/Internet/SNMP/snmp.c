...
int32_t snmpd_run(void)
{
	...
	switch(getSn_SR(SOCK_SNMP_AGENT))
	{
		case SOCK_UDP :
			...
			{
				request_msg.len= recvfrom(SOCK_SNMP_AGENT, request_msg.buffer, len, svr_addr, &svr_port, &svr_addr_len);
			}
			else
			{
				request_msg.len = 0;
			...
			if (request_msg.len > 0)
			{
				...
				{
					sendto(SOCK_SNMP_AGENT, response_msg.buffer, response_msg.index, svr_addr, svr_port, &addr_len);
				}
...
int32_t snmp_sendTrap(uint8_t * managerIP, uint8_t * agentIP, int8_t* community, dataEntryType enterprise_oid, uint32_t genericTrap, uint32_t specificTrap, uint32_t va_count, ...)
{
	...
	// Send SNMP Trap Packet to NMS
	{
		...
		sendto(SOCK_SNMP_TRAP, packet_trap, packet_index, managerIP, PORT_SNMP_TRAP, &addr_len);
